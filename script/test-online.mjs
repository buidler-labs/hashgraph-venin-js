...
if (
  ...
  !process.env.PORTAL_NETWORK
) {
  throw new Error(
    "Please set PORTAL_EMAIL, PORTAL_PASSWORD and PORTAL_NETWORK to be able to run on official networks."
  );
}
...
const tokenResponse = await axios.post(
  "https://api.portal.hedera.com/v1/token",
  {
...
const meResponse = await axios.get("https://api.portal.hedera.com/v1/user/me", {
  headers: { cookie },
});
const me = meResponse.data;
const networkOfInterest = me.networks.find(
  (checkedNetwork) => checkedNetwork.label === network
);
const accountResponse = await axios.get(
  `https://api.portal.hedera.com/v1/testnet/${networkOfInterest.id}/account`,
  {
    headers: { cookie },
  }
);
...
// // Run the test
console.log(
  `Running test:node for account '${accountId}' on the '${network}' network.`
);
