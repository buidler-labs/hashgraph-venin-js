...
export class WalletType {
  public constructor(
    ...
    public readonly defaultController: new (
      ctx: StratoContext
    ) => WalletController = ImpotentWalletController,
    public readonly providerHaving: new (
      ctx: StratoContext,
      controller: WalletControllerEvents
    ) => WalletProvider = NotSupportedWalletProvider,
    public readonly computeColdStartOptionsFrom?: (
      params: StratoParameters
    ) => any
  ) {
...
export class WalletTypes {
  ...
  public getBy({ name }: { name: string }): WalletType {
    const candidateWalletTypes = this.knownWalletTypes.filter(
      (type) => type.name === name
    );

    return candidateWalletTypes.length === 0
      ? this.Unknown
      : candidateWalletTypes[0];
  }
}
