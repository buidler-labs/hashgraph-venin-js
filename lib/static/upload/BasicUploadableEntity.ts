...
> implements UploadableEntity<T, R>
{
  ...
  public async uploadTo({
    session,
    args = [],
  }: ArgumentsForUpload): Promise<T> {
    const whatToUpload = await this.getContent();
    const { appendTxCount, areOverridesProvided, fileTransactions } =
    ...
    if (transactionReceipt.status !== Status.Success) {
      ...
      if (
        ...
        fileTransactions[1] instanceof FileAppendTransaction
      ) {
        session.log.debug(
  ...
  private async _getFileTransactionsFor({
    content,
    session,
    args = [],
  }: ArgumentsToGetFileTransaction) {
    const fileChunkSize: number = session.network.defaults.fileChunkSize;
    const fileTransactions: Array<
    ...
    fileTransactions.push(
      new FileCreateTransaction(
        Object.assign(
          ...
          {
            contents:
              content.length > fileChunkSize
                ? content.slice(0, fileChunkSize)
                : content,
          }
        )
      )
  ...
  protected abstract onFileUploaded({
    session,
    receipt,
    args,
  }: ArgumentsOnFileUploaded): Promise<T>;
}
