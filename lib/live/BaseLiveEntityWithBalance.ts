import { AccountBalance, AccountBalanceQuery, Hbar, Status, TokenAssociateTransaction, TokenId, TransferTransaction } from "@hashgraph/sdk";
...
export abstract class BaseLiveEntityWithBalance<T, I, P> extends LiveEntity<T, I, P> {
  ...
  public transferHbarToLiveEntity(hbarAmount: number): Promise<Status> {
    const transferTransaction = new TransferTransaction()
      .addHbarTransfer(this.session.accountId, new Hbar(-hbarAmount))
      .addHbarTransfer(this.id.toString(), new Hbar(hbarAmount));
