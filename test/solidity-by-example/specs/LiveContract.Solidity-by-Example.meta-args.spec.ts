...
describe('LiveContract.Solidity-by-Example.meta-arguments', () => {
  it('when calling a non state-changing contract method (aka `a query`), passing in meta-arguments should get propagated to the actual transaction', async () => {
    ...
    const metaArgs = {
      ...
      paymentTransactionId: TransactionId.generate(
    ...
    expect(sessionExecuteSpy.mock.calls[0][0]).toBeInstanceOf(
      ContractCallQuery
    );

    const queryTransaction = sessionExecuteSpy.mock
      .calls[0][0] as any as ContractCallQuery;
  ...
  it('when calling a state-changing contract method, passing in meta-arguments should get propagated to the actual transaction', async () => {
    ...
    const executeTransaction = sessionExecuteSpy.mock
      .calls[0][0] as any as ContractExecuteTransaction;

    expect(executeTransaction.payableAmount).toEqual(metaArgs.amount);
    expect(executeTransaction.maxTransactionFee).toEqual(
      metaArgs.maxTransactionFee
    );
    expect(executeTransaction.transactionId).toEqual(metaArgs.transactionId);
    expect(executeTransaction.transactionMemo).toEqual(
      metaArgs.transactionMemo
    );
    expect(executeTransaction.transactionValidDuration).toEqual(
      metaArgs.transactionValidDuration
    );
  });
