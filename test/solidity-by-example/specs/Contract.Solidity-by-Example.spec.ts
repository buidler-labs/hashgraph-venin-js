...
describe('Contract.Solidity-by-Example', () => {
    ...
    it('given a valid solidity contract, serializing and then deserializing it should give back the same contract', async () => {
        const originalHelloWorldContract = await Contract.newFrom({ code: read({ contract: 'hello_world' }) });
    ...
    it("given a 2x solidity contract code, retrieving entrie by their name should be possible", async () => {
        const receiverContract = await Contract.newFrom({ code: read({ contract: 'call' }), ignoreWarnings: true, name: 'Receiver' });
        const callerContract = await Contract.newFrom({ code: read({ contract: 'call' }), ignoreWarnings: true, name: 'Caller' });
