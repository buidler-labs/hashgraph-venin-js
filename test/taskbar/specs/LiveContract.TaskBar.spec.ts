...
describe("LiveContract.TaskBar", () => {
  it("given the taskbar use-case contracts, initializing a task should allow for getting it back later on", async () => {
    ...
    await expect(
      taskRegistryLiveContract.initializeTask(
        { gas: 200_000 },
        taskId,
        100,
    ...
    const gottenTask = await taskRegistryLiveContract.getTask(
  ...
  it.skip("given the taskbar contracts, do stuff", async () => {
    ...
    const taskRegistry = await taskRegistryManagerLiveContract.registryWithSlot(
      { gas: 250000 }
    );
