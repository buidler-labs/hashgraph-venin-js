...
async function loadContractRegistry(
  ...
  const originalContractsRelativePathEnv =
    process.env.HEDERAS_CONTRACTS_RELATIVE_PATH;
  const originalContractRegistryRecurseEnv =
    process.env.HEDERAS_CONTRACT_REGISTRY_RECURSE;
  ...
  const toReturn = returnDefault
    ...
    : new ContractRegistry(path, recurse);

  jest.resetModules();
  process.env.HEDERAS_CONTRACTS_RELATIVE_PATH =
    originalContractsRelativePathEnv;
  process.env.HEDERAS_CONTRACT_REGISTRY_RECURSE =
    originalContractRegistryRecurseEnv;
